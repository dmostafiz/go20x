	...
	<div class="card-header">
		<div class="flex-between-center row">
			<div class="d-flex align-items-center pe-0 col-sm-auto col-4">
				<h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Orders</h5></div>
								...
								<table role="table" class="fs--1 mb-0 overflow-hidden table table-sm table-striped">
									...
									<tbody>
										...
										<tr class="align-middle white-space-nowrap" role="row">
											...
											<td>#{{ ($trx->id) }}</td>
											<td>${{ ($trx->order_total) }} </td>
											<td>${{ ($trx->shipping_charges) }}</td>
											<td>{{ (!is_null($trx->vat_tax) ? '$'.$trx->vat_tax : '-') }}</td>
											<td><div class="d-inline-block badge badge-soft-success rounded-pill text-capitalize">{{ $trx->status }}</div> {{-- <br>{{\Carbon\Carbon::parse($trx->updated_at)->diffForHumans() }} --}}</td>

											<td>{{ ($trx->created_at) }}</td>

											<td role="cell" class="text-end">
											
											<a href="{{route('detail',$trx->id)}}" class="btn btn-sm btn-info" data-toggle="tooltip" title="" data-original-title="Detail">
