...
@section('content')
	...
	<div class="card-header">
		<div class="flex-between-center row">
			<div class="d-flex align-items-center pe-0 col-sm-auto col-4">
				<h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Shipping Address</h5></div>
								...
								<table role="table" class="fs--1 mb-0 overflow-hidden table table-sm table-striped">
									...
									<tbody>
										...
										<tr class="align-middle white-space-nowrap" role="row">
											...
											<td>{{$address->address_line_1}} {{$address->address}} </td>
											<td>{{$state}} </td>
											<td>{{$country}} </td>

											<td role="cell" class="text-end">
											<a href="{{route('edit.shipping',$address->id)}}" class="btn btn-sm btn-primary" data-toggle="tooltip" title="" data-original-title="Detail">
                                                <i class="fa fa-pen"></i>
                                            </a>
											<a href="{{route('destroy.shipping',$address->id)}}" onclick="return confirm('Are you sure you want to delete this ?');" class="btn btn-sm btn-danger" data-toggle="tooltip" title="" data-original-title="Detail">
	...
	<div class="card-footer">
		<div class="d-flex justify-content-center align-items-center">
			   {{-- {{ $shipping_address->links('pagination::bootstrap-4') }} --}}
		</div>
	</div>
</div>
@endsection
