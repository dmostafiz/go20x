...
function getPersonalUsers($id){
    $personals = Pairing::where('sponsor_id',$id)->get()->toArray();
    return $personals;
}

function getSponsorWithOrder($id, $counter = 0)
{   
    if($id == 1){
       return 1;
    }   
    ...
    $user = DB::table('users_old')->where('id',$id)->first();
    if(is_null($user)){

        return 0;
    }

    $order = Order::where('user_id',$user->ref_by)->where('created_at', '>', Carbon::now()->subDays(60))->first();
    
    if($counter == 1){

        //dd($id, Carbon::now()->subDays(60), $order);
    }

    if (is_null($order)) {
         
      //$user = Pairing::where('uid',$id)->first();
      $user = DB::table('pairing_old')->where('uid',$id)->first();
      
      if (!is_null($user)) {
          return getSponsorWithOrder($user->sponsor_id, 1);
      }
    }else{
...
function sendRegisterEmail($user, $type = null, $shortCodes = [])
{
    $general = GeneralSetting::first();

    $emailTemplate = EmailTemplate::where('act', 'REGISTER')->where('email_status', 1)->first();
    if ($general->en != 1 || !$emailTemplate) {
        return;
    }


    $message = 'Welcome to Legacy Bloom Gifting';
    $message = shortCodeReplacer("{{username}}", $user->username, $message);
    $message = shortCodeReplacer("{{message}}", $emailTemplate->email_body, $message);
...
function GetValidParentId($sponsor_id)
{

    $userid = User::where('id', $sponsor_id)->first();

    $parent_id = $userid->id;
    $chkqry21 = User::where(['position_id' => $parent_id])->count();

    if ($chkqry21 > 1) {
        $chkqry = User::where(['position_id' => $parent_id])->get();
        dd($chkqry);
        foreach ($chkqry as $chkqrys) {
            $chkqry22 = User::where(['position_id' => $chkqrys->id])->count();

            if ($chkqry22 < 2) {

                return GetValidParentId($chkqrys->id);
            }
        }
    } else {
        return $userid->id;
...
function ShippingTaxSettings()
{
    $settings = \DB::table('shipping_tax_settings')->first();
    return $settings;
}
...
//     $parent_id=$userid->id;
//     //$chkqry21 = User::where(['position_id'=>$parent_id])->count();
//     $chkqry = User::where(['position_id'=>$sponsor_id])->get();

//     $par_id=[];
//     foreach($chkqry as $chkqrys)
//     {
//         $chkqry21 = User::where(['position_id'=>$chkqrys->id])->count();
//         echo $chkqry21."<br>";
//         if($chkqry21>1){
...
{
    $position = User::where('position_id', $parent_id)->get();
    if ($position->count() != 0) {
...
// function CheckParentPaid($cv,$parent_id){
//     if($parent_id != 1){
//         $date = Carbon::now()->subDays(30);
//         $users = Purchase::where('user_id', $parent_id)->where('created_at', '>=', $date)->get();
//         $wordCount = $users->count();
//         if ($wordCount > 0) {
//             $level = 1;
//             $lev_payout = SetMatrix($cv,$parent_id,$level);
//         }
//         else {
//             $parentdata=Pairing::where('uid',$parent_id)->first();
//             $nparent_id=$parentdata->parent_id;
//             $check = CheckParentPaid($cv,$nparent_id);
//         }
//     }
//     else {
//         echo "test";
...
function formatTree(&$childs, $tree, $parent)
{
    foreach ($tree as $item) {
        if ($item['parent_id'] == $parent) {
            $childs[] = $item;
            formatTree($childs, $tree, $item['uid']);
        }
    }
}

function getRecursiveChildren($id)
{
    $childs = array();
    $tree = Pairing::get()->toArray();
    formatTree($childs, $tree, $id);
    return $childs;
}
...
function getOrderBySubscription($id)
{
    $order = Order::where('subscription_id', $id)->first();
    return $order;
}

function getProductBySubscription($id)
{
    $order = Order::where('subscription_id', $id)->first();
    $result = array();
    if (!is_null($order)) {
...
function getStateByISO2($iso, $country)
{
...
function getCountryByISO2($country)
{
...
function getTaxByState($country, $state, $contrl = null)
{
    ...
    if (!is_null($states)) {
        ...
        if (!is_null($states->vat_tax)) {

            if (!is_null($contrl)) {
                return $states->vat_tax;
            } else {
            ...
            $shippingTax = ShippingTaxSettings();

            $tax = 0;
            if ($country == 'AU') {
                $tax = (float)$shippingTax->au_tax;
            }
            if ($country == 'NZ') {
                $tax = (float)$shippingTax->nz_tax;
            }
            if ($country == 'GB') {
                $tax = (float)$shippingTax->uk_tax;
            }
...
function getUserWalletBalance()
{
    if (Auth::check()) {
        $userId = Auth::user()->id;
        $wallet = User::where('id', $userId)->first();
...
function getSingleOrderDetailsById($orderId)
{
...
function get_product_single($id){
     $orders = \DB::table('products')->where('id',$id)->first();
